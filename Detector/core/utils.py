...
def load_config(config_params):
    if config_params['tiny']:
        STRIDES = np.array(cfg.YOLO.STRIDES_TINY)
        ANCHORS = get_anchors(cfg.YOLO.ANCHORS_TINY, config_params['tiny'])
        XYSCALE = cfg.YOLO.XYSCALE_TINY if config_params['model'] == 'yolov4' else [1, 1]
    else:
        STRIDES = np.array(cfg.YOLO.STRIDES)
        if config_params['model'] == 'yolov4':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS, config_params['tiny'])
        elif config_params['model'] == 'yolov3':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS_V3, config_params['tiny'])
        XYSCALE = cfg.YOLO.XYSCALE if config_params['model'] == 'yolov4' else [1, 1, 1]
